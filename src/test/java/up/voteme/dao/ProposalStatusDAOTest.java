...
public class ProposalStatusDAOTest {
	...
	public void A_findAllTest() {
		...
		List<ProposalStatus> list = dao.findAll();
	...
	public void B_storeTest(){
		...
		List<ProposalStatus> beforList = dao.findAll();
		//modify item
		ProposalStatus item = dao.findById(1L);
		item.setId(0);
		item.setStatus("OHOHOHOHOHO");
		
		long id =  dao.store(item);
		List<ProposalStatus> afterList = dao.findAll();
	...
	public void C_findByIdTest() {
		...
		ProposalStatus item = dao.findById(id);
