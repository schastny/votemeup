	            ...
	            password:{
	            	validators: {
	                    ...
	                    stringLength: {
	                    	min: 5,
	                    	max: 70,
                            message: 'Это поле должно иметь длину от 5 до 70 символов и содержать только символы латинского алфавита  или цифры !',
                       },
                       regexp: {
	    ...
	    $("#userEmail").change(function() {
	    	...
	    	if(usEmail.length <= 4){
	    		...
	    		if(!pattern.test(usEmail)){
	    			emailError();
	    			$("#emailError").text("");
	  				$("#emailError").text("Укажите корректный e-mailадрес!");	    			
	    		}
	    		else{
	    			$.ajax({
		  	    		  type: "POST",
